...
namespace PixelCollector.Networking.Server
{
  ...
  /// </summary>
  public delegate CommandResponse CommandHandler(SocketIOUnity socket, object args);
  ...
  public class SocketCommandHandler
  {
    ...
    public void HandleCommand(SocketIOResponse response)
    {
      try
      {
        ...
        if (data == null)
        {
          ...
          SendResponse(CommandResponse.ServerError("수신한 명령어 데이터가 null입니다."));
          return;
        }
    ...
    public void ExecuteCommand(string command, object args)
    {
      ...
      else
      {
        ...
        SendResponse(CommandResponse.NotFound(command));
      }
    }
